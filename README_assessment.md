...
1. Top-3 Experts: When we set enforce_eager = True, we see top-3 to be experts 43, 7, and 58 (similar to when enforce_eager = False, but now the distribution is way more balanced)
...
2. Normalized Distribution: top 4 experts account for ~9% of the selection. mean per expert in uniform dist: 100/60 = 1.67% (vs. 3.17% for top expert, indicates good balancing), std dev: 0.44% (low, good balance), entropy: 5.86 (max = 6, good)

3. Metrics (on real data) -> Entropy: 5.8600, CV: 0.3765, Utilization: 93.75%
